...
import {
  AboutKoboRide,
  WhatsAppSupport,
} from "@/components/support/WhatsAppSupport";
  ...
  if (!authenticated) {
    return (
      <div className="flex min-h-full flex-col items-center bg-[#FAFAF7] px-6 pt-[max(4rem,calc(env(safe-area-inset-top)+2.5rem))] text-center">
        ...
        <AboutKoboRide className="mt-8" />
        <WhatsAppSupport className="mt-3 text-left" />
      </div>
    );
  }
    ...
    <div className="min-h-full bg-[#FAFAF7] px-4 pt-[max(1.25rem,env(safe-area-inset-top))] pb-4">
      ...
      <section className="mt-6">
        ...
        <p className="mt-1 text-[14px] text-[#8A8780]">
          Alerts turn on when you sign in or book. Chrome will ask once.
        </p>
      ...
      <AboutKoboRide className="mt-6" />
      <WhatsAppSupport className="mt-3" />
